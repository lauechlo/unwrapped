  ...
  return (
    <section className="py-12 md:py-20 px-4 md:px-8 bg-black">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-bold mb-8 md:mb-12 text-center px-2">Your Listening Patterns</h2>
        ...
        <div className="grid gap-6 md:gap-10">
          {cards.map((card, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`bg-gradient-to-br ${theme.gradient} border-2 ${theme.border} p-4 md:p-10 rounded-xl md:rounded-2xl transition-all`}
              >
                {/* Pattern Label - Viral */}
                <div className="mb-4 md:mb-6">
                  ...
                  <h3 className="text-xl sm:text-2xl md:text-2xl font-bold text-white mb-3 md:mb-4 leading-tight">
                    {card.patternLabel}
                  </h3>
                </div>
                ...
                <div className={`space-y-2 md:space-y-3 mb-4 md:mb-6 pl-3 md:pl-4 border-l-2 ${theme.border.split(' ')[0].replace('border-', 'border-l-')}`}>
                  <div className="flex items-start gap-2 md:gap-3">
                    <span className={`${theme.accent} text-xs md:text-sm font-mono flex-shrink-0`}>├─ KEY FINDING:</span>
                    <p className="text-gray-300 text-sm md:text-base leading-relaxed">{card.core}</p>
                  </div>
                  <div className="flex items-start gap-2 md:gap-3">
                    <span className={`${theme.accent} text-xs md:text-sm font-mono opacity-80 flex-shrink-0`}>├─ WHY:</span>
                    <p className="text-gray-300 text-sm md:text-base leading-relaxed">{card.supporting}</p>
                  </div>
                  <div className="flex items-start gap-2 md:gap-3">
                    <span className={`${theme.accent} text-xs md:text-sm font-mono opacity-60 flex-shrink-0`}>└─ WHAT IT MEANS:</span>
                    <p className="text-gray-300 text-sm md:text-base leading-relaxed">{card.behavior}</p>
                  </div>
                </div>
                ...
                {/* Show Proof Button - Showy and Prominent - Minimum 44px tap target */}
                <button
